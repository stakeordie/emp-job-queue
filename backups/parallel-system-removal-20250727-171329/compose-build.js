...
class ComposeBuilder {
  ...
  async parseWorkerSpec(workerSpec, options = {}) {
    ...
    const connectors = workerSpec.split(',').map(spec => {
      ...
      const connectorConfig = serviceMapping.connectors[connector];
      if (!connectorConfig) {
        throw new Error(`Unknown connector: ${connector}. Check service-mapping.json`);
      }
      
      return {
        connector: connector.trim(),
        count: parseInt(count),
        binding: connectorConfig.resource_binding
      };
    });
  ...
  async generateEnvironmentForWorkerSpec(workerSpec) {
    const serviceMapping = await this.loadServiceMapping();
    const environment = {
      'WORKER_CONNECTORS': workerSpec.raw
    };

    // Collect all required and optional environment variables for all connectors
    for (const connector of workerSpec.connectors) {
      const connectorConfig = serviceMapping.connectors[connector.connector];
      if (!connectorConfig) continue;

      // Add required environment variables
      if (connectorConfig.required_env) {
        for (const envVar of connectorConfig.required_env) {
          // Use ${VAR} format for Docker Compose variable substitution
          environment[envVar] = `\${${envVar}}`;
        }
      }

      // Add optional environment variables
      if (connectorConfig.optional_env) {
        for (const envVar of connectorConfig.optional_env) {
          environment[envVar] = `\${${envVar}}`;
        }
      }
    }
  ...
  async generateDockerTarget(connectors) {
    ...
    const stageNames = connectors.map(c => {
      const connectorConfig = serviceMapping.connectors[c.connector];
      if (connectorConfig && connectorConfig.build_stage) {
        return connectorConfig.build_stage;
      }
    ...
    // Remove duplicates and return unique stages joined
    const uniqueStages = [...new Set(stageNames)];
    return uniqueStages.join('-');
  }
  ...
  displaySummary(workerSpec, options = {}) {
    ...
    console.log(`  WORKER_CONNECTORS="${workerSpec.raw}" pnpm machine:build`);
