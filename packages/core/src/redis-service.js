...
export class RedisService {
    ...
    eventBroadcaster;
    constructor(redisUrl, eventBroadcaster) {
    ...
    async failJob(jobId, error, canRetry = true) {
        try {
            ...
            if (shouldRetry) {
                ...
                logger.info(`Job ${jobId} failed but will retry (${newRetryCount}/${job.max_retries}): ${error}`);
            }
            else {
    ...
    async getNextJob(workerCapabilities) {
        try {
            ...
            for (const jobCandidate of eligibleJobs) {
                ...
                if (claimed) {
                    logger.debug(`Worker ${workerCapabilities.worker_id} successfully claimed job ${job.id} (any-worker-any-job mode)`);
                    return job;
                }
    ...
    async canWorkerHandleJob(job, capabilities) {
        ...
        if (job.requirements?.models &&
            ...
            if (workerModels && workerModels !== 'all') {
                ...
                const hasRequiredModels = job.requirements.models.every(model => availableModels.includes(model));
    ...
    async updateWorkerHeartbeat(workerId, systemInfo) {
        const now = new Date().toISOString();
    ...
    async getCompletedJobs(limit = 50) {
        try {
     ...
     */
    async detectAndFixOrphanedJobs() {
        try {
            let fixedCount = 0;
                ...
                if (!activeWorkerIds.has(workerId)) {
                    ...
                    for (const [jobId, jobDataStr] of Object.entries(orphanedJobs)) {
                        try {
                            ...
                            const effectiveDateTime = resetJob.workflow_datetime || Date.parse(resetJob.created_at);
                            const score = effectivePriority * 1000000 + (Number.MAX_SAFE_INTEGER - effectiveDateTime);
                            await this.redis.zadd('jobs:pending', score, jobId);
            ...
            for (const worker of activeWorkers) {
                ...
                if (currentJobId && currentJobId !== 'none' && currentJobId !== '') {
                    ...
                    if (jobStatus === 'cancelled' || jobStatus === 'completed' || jobStatus === 'failed') {
                        logger.warn(`Worker ${worker.worker_id} stuck on ${jobStatus} job ${currentJobId} - clearing worker state`);
                        ...
                        fixedCount++;
                        logger.info(`Cleared stuck worker ${worker.worker_id} from ${jobStatus} job ${currentJobId}`);
                    }
                }
            }
    ...
    async detectStuckJobs() {
        try {
            ...
            for (const job of activeJobs) {
                if (!job.worker_id || !job.assigned_at)
                    continue;
                ...
                if (timeSinceHeartbeat > heartbeatTimeoutSec) {
                    logger.warn(`Worker ${job.worker_id} heartbeat timeout (${Math.round(timeSinceHeartbeat)}s > ${heartbeatTimeoutSec}s) - releasing stuck job ${job.id}`);
                    await this.releaseStuckJob(job, `Worker heartbeat timeout (${Math.round(timeSinceHeartbeat)}s)`);
                    fixedCount++;
                    continue;
                }
                ...
                if (timeSinceJobStart > jobProgressTimeoutSec) {
                    ...
                    if (progressData.updated_at) {
                        ...
                        if (timeSinceProgress > jobProgressTimeoutSec) {
                            logger.warn(`Job ${job.id} stuck - no progress for ${Math.round(timeSinceProgress)}s - releasing from worker ${job.worker_id}`);
                            await this.releaseStuckJob(job, `No progress timeout (${Math.round(timeSinceProgress)}s)`);
                            fixedCount++;
                        }
                    }
                    else {
                        // No progress data at all after a long time
                        logger.warn(`Job ${job.id} stuck - no progress data after ${Math.round(timeSinceJobStart)}s - releasing from worker ${job.worker_id}`);
                        await this.releaseStuckJob(job, `No progress data timeout (${Math.round(timeSinceJobStart)}s)`);
                        fixedCount++;
                    }
                }
            }
     ...
     */
    async releaseStuckJob(job, reason) {
        try {
            const newRetryCount = job.retry_count + 1;
            ...
            if (newRetryCount >= maxRetries) {
                ...
                logger.warn(`Job ${job.id} exceeded max retries (${newRetryCount}/${maxRetries}) - marking as failed`);
                ...
                logger.info(`Job ${job.id} will be retried (${newRetryCount}/${maxRetries}) - returning to queue`);
    ...
    async searchJobs(filter, page = 1, pageSize = 20) {
        return { jobs: [], total_count: 0, page, page_size: pageSize, has_more: false };
    }
    async getJobsByStatus(status, _limit = 50) {
        return [];
    }
    async getJobsByWorker(_workerId, _limit = 50) {
        return [];
    }
    async getJobsByCustomer(_customerId, _limit = 50) {
        return [];
    }
    async updateWorkerCapabilities(_workerId, _capabilities) { }
    async updateWorkerStatus(workerId, status, currentJobs) {
        const now = new Date().toISOString();
        ...
        if (this.eventBroadcaster && oldStatus !== status) {
            this.eventBroadcaster.broadcastWorkerStatusChanged(workerId, oldStatus, status, updateData.current_job_id || undefined);
        }
        logger.debug(`Worker ${workerId} status updated to ${status}, current_job_id: ${updateData.current_job_id || 'none'}`);
    }
    async removeWorker(_workerId) { }
    async getWorker(_workerId) {
        return null;
    }
    async getAllWorkers() {
        return [];
    ...
    async getIdleWorkers() {
        return [];
    }
    async findCapableWorkers(_jobRequirements) {
        return [];
    }
    async searchWorkers(_filter) {
        return [];
    }
    async getWorkerLastHeartbeat(_workerId) {
        return null;
    }
    async cleanupStaleWorkers(_timeoutSeconds) {
        return [];
    }
    ...
    async cleanupCompletedJobs(_olderThanHours) {
        return 0;
    }
    async cleanupFailedJobs(_olderThanDays) {
        return 0;
    }
    async optimizeJobQueue() { }
    async exportJobData(_startDate, _endDate) {
        return [];
    }
    async importJobData(_jobs) {
        return 0;
    }
    async getRedisInfo() {
        return {};
