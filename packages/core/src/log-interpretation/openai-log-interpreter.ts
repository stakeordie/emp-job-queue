...
export class OpenAILogInterpreter extends BaseLogInterpreter {
  ...
  private registerOpenAIPatterns(): void {
    ...
    this.registerPattern(createCustomPattern(
      ...
      (match: RegExpMatchArray | string, context?: LogContext) => ({
        userMessage: 'OpenAI job completed but no images were generated. This usually indicates your request was rejected by content policy or an internal processing error.',
        severity: 'error' as const,
        category: 'service' as const,
        suggestedAction: 'Try modifying your prompt to comply with OpenAI policies, or try again with a different request',
        retryRecommended: false,
   ...
   */
  async interpretAPIResponse(response: any, context?: LogContext): Promise<InterpretedMessage | null> {
    try {
   ...
   */
  private interpretStatusCode(status: number, statusText?: string, context?: LogContext): InterpretedMessage {
    switch (status) {
      case 401:
        return {
