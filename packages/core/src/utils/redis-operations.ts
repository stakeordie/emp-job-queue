...
export class RedisOperations {
   ...
   */
  static async scanKeys(redis: Redis, pattern: string): Promise<string[]> {
    const keys: string[] = [];
    let cursor = '0';
  ...
  static deserializeJobFromRedis(jobData: Record<string, string>): Job | null {
    ...
    try {
      ...
      // Optional fields
      if (jobData.customer_id) job.customer_id = jobData.customer_id;
      ...
      if (jobData.failed_at) job.failed_at = jobData.failed_at;
      if (jobData.worker_id) job.worker_id = jobData.worker_id;
      if (jobData.last_failed_worker) job.last_failed_worker = jobData.last_failed_worker;
      if (jobData.processing_time) job.processing_time = parseFloat(jobData.processing_time);
      if (jobData.estimated_completion) job.estimated_completion = jobData.estimated_completion;
   ...
   */
  static async storeJob(redis: Redis, job: Job): Promise<void> {
   ...
   */
  static async getJobsFromKeys(redis: Redis, keys: string[]): Promise<Job[]> {
