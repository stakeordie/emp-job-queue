...
export declare class JobBroker implements JobBrokerInterface {
     ...
     */
    updateWorkflowStatus(workflowId: string, status: 'active' | 'completed' | 'failed'): Promise<void>;
    /**
     ...
     */
    archiveCompletedJobs(olderThanMinutes?: number, archiveDir?: string): Promise<{
        archived: number;
        errors: number;
    }>;
    /**
     ...
     */
    getArchivedJobs(status: 'completed' | 'failed', startDate: string, endDate: string, archiveDir?: string): Promise<unknown[]>;
    /**
     * Clean up archive files older than specified days
     */
    cleanupArchives(olderThanDays?: number, archiveDir?: string): Promise<{
        deleted: number;
        errors: number;
    }>;
}
