...
export class ComfyUIConnector implements ConnectorInterface {
  ...
  constructor(connectorId: string) {
    ...
    this.config = {
      ...
      service_type: this.service_type as 'comfyui',
      ...
      settings: {
        ...
        image_format: (process.env.WORKER_COMFYUI_IMAGE_FORMAT as 'png' | 'jpg' | 'webp') || 'png',
    ...
    this.httpClient = axios.create({
      ...
      auth:
        this.config.auth?.type === 'basic' && this.config.auth.username && this.config.auth.password
          ? {
  ...
  private async waitForCompletion(
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(
        ...
        (this.config.settings.workflow_timeout_seconds || 600) * 1000
      );
