...
export class RestAsyncConnector implements ConnectorInterface {
  ...
  constructor(connectorId: string) {
    ...
    this.config = {
      ...
      settings: {
        method: (process.env.WORKER_REST_ASYNC_METHOD as any) || 'POST',
        response_format: (process.env.WORKER_REST_ASYNC_RESPONSE_FORMAT as any) || 'json',
  ...
  async initialize(): Promise<void> {
    logger.info(`Initializing REST Async connector ${this.connector_id} at ${this.config.base_url}`);
    logger.info(`REST Async settings: ${this.config.settings.method} requests with ${this.config.settings.polling_interval_ms}ms polling`);
  }
  ...
  async canProcessJob(jobData: JobData): Promise<boolean> {
    return jobData.type === 'rest_async' || 
      ...
      return {
        ...
        service_metadata: {
          ...
          processing_stats: {
            ...
            method,
            job_token: jobToken,
            polling_cycles: Math.floor(processingTime / this.config.settings.polling_interval_ms!)
          }
        }
      };
  ...
  private extractJobToken(response: any): string {
    // Try various common patterns for job tokens/IDs
    const token = response.job_id || 
                  response.jobId || 
                  response.id || 
                  response.token || 
                  response.task_id ||
  ...
  private async pollForCompletion(
    ...
    const maxPolls = Math.floor(this.config.timeout_seconds * 1000 / this.config.settings.polling_interval_ms!);
    let polls = 0;
    ...
    while (polls < maxPolls) {
      try {
        ...
        await this.sleep(this.config.settings.polling_interval_ms!);
        polls++;
    ...
    throw new Error(`Async job ${jobToken} timed out after ${maxPolls * this.config.settings.polling_interval_ms! / 1000} seconds`);
  }

  private extractStatus(data: any): string {
    const statusField = this.config.settings.status_field!;
    return data[statusField] || data.status || 'unknown';
  }

  private extractProgress(data: any): number {
  ...
  private extractResult(data: any): any {
    const resultField = this.config.settings.result_field!;
    return data[resultField] || data.result || data.data || data;
  }

  private extractError(data: any): string {
    return data.error || data.error_message || data.message || 'Unknown error';
  }
  ...
  private estimateCompletion(progress: number, polls: number): number | undefined {
    ...
    const timePerPercent = (polls * this.config.settings.polling_interval_ms!) / progress;
