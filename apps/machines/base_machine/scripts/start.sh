...
setup_redis_workers() {
    ...
    local release_url="https://api.github.com/repos/stakeordie/emp-job-queue/releases/latest"
    local download_url
    ...
    # Download with wget
    if ! wget -O "$tarball_path" "$download_url" 2>&1 | tee -a "$LOG_FILE"; then
    ...
    log "Successfully downloaded worker package ($(stat -c%s "$tarball_path" 2>/dev/null || stat -f%z "$tarball_path" 2>/dev/null || echo "unknown") bytes)"
