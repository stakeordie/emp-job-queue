/**
 * Telemetry Pipeline Meta Tests (API)
 *
 ...
 *
 * Test Flow:
 * 1. Services start correctly (API, OTLP Collector)
...
describe('API Telemetry Pipeline Meta Tests', () => {
  ...
  it('should create telemetry client and send test event to collector', async () => {
    ...
    // Send test event
    testClient.addEvent('meta.test.ping', {
      'test_id': `meta-test-${Date.now()}`,
  ...
  it('should verify collector forwards to Dash0', async () => {
    ...
    // Send a test event
    const testClient = createTelemetryClient({
      serviceName: 'test-meta-api-dash0',
      serviceVersion: '1.0.0',
      collectorEndpoint: COLLECTOR_URL,
      environment: 'test'
    });

    const testId = `dash0-test-${Date.now()}`;
    testClient.addEvent('meta.test.dash0', {
      'test_id': testId,
