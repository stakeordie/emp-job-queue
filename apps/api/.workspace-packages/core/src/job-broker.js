...
export class JobBroker {
    ...
    async submitJob(request) {
        ...
        if (workflowId) {
            ...
            if (metadata) {
                ...
                logger.info(`Job ${jobId} inheriting from workflow ${workflowId}: priority=${workflowPriority}, datetime=${workflowDatetime}`);
            }
            else {
        ...
        const submittedJobId = await this.submitJobToRedis(jobId, job, workflowPriority, workflowDatetime);
        logger.info(`Job ${submittedJobId} submitted with workflow priority ${workflowPriority}, datetime ${workflowDatetime}`);
        return submittedJobId;
    }
    /**
     * Enhanced job submission to Redis with workflow-based scoring
     */
    async submitJobToRedis(jobId, jobData, workflowPriority, workflowDatetime) {
        ...
        logger.debug(`Job ${jobId} added to queue with score ${score} (priority=${workflowPriority}, datetime=${workflowDatetime})`);
        return jobId;
    }
    /**
     ...
     */
    async getNextJobForWorker(workerCapabilities) {
        ...
        if (claimed) {
            logger.info(`Worker ${workerCapabilities.worker_id} claimed job ${jobId} (any-worker-any-job mode)`);
            return job;
        }
        else {
     ...
     */
    async requeueUnworkableJob(jobId) {
        const job = await this.redis.getJob(jobId);
    ...
    async releaseJob(jobId) {
        ...
        const score = (job.workflow_priority || job.priority) * 1000000 +
     ...
     */
    async createWorkflow(priority, customerId, workflowId) {
    ...
    async getAllJobs() {
        try {
                ...
                if (pendingJobsData) {
                    for (let i = 0; i < pendingJobs.length; i++) {
                        ...
                        if (result && !result[0] && result[1] && Object.keys(result[1]).length > 0) {
                            jobs.push(this.parseJobData(pendingJobs[i], result[1], 'pending'));
                        }
                    }
                }
            }
            ...
            let cursor = '0';
            do {
                const result = await this.redis['redis'].scan(cursor, 'MATCH', 'jobs:active:*', 'COUNT', 100);
                cursor = result[0];
     ...
     */
    parseJobData(jobId, jobData, status) {
        ...
        return {
            ...
            created_at: parseInt(jobData.created_at || Date.now().toString()),
            assigned_at: jobData.assigned_at ? parseInt(jobData.assigned_at) : undefined,
            started_at: jobData.started_at ? parseInt(jobData.started_at) : undefined,
            completed_at: jobData.completed_at ? parseInt(jobData.completed_at) : undefined,
            worker_id: jobData.worker_id,
            progress: jobData.progress ? parseInt(jobData.progress) : undefined,
            result: jobData.result,
            error: jobData.error,
            failure_count: parseInt(jobData.failure_count || '0'),
        };
    }
    /**
     ...
     */
    async archiveCompletedJobs(olderThanMinutes = 5, archiveDir = './data/archived-jobs') {
     ...
     */
    async archiveJobsByStatus(status, cutoffTime, archiveDir) {
        ...
        try {
            ...
            if (Object.keys(jobsToArchive).length > 0) {
                ...
                logger.info(`Archived ${archivedCount} ${status} jobs, kept ${Object.keys(jobsToKeep).length} recent jobs`);
            }
        }
        catch (error) {
     ...
     */
    async writeJobsToArchive(jobs, status, archiveDir) {
        // Group jobs by date for partitioning
        const jobsByDate = {};
        for (const [jobId, jobData] of Object.entries(jobs)) {
            const job = jobData;
            const completedAt = job.completed_at || job.created_at || Date.now();
        ...
        for (const [date, jobEntries] of Object.entries(jobsByDate)) {
            ...
            const lines = jobEntries.map(([jobId, jobData]) => JSON.stringify({ id: jobId, ...jobData }));
            try {
     ...
     */
    async getArchivedJobs(status, startDate, endDate, archiveDir = './data/archived-jobs') {
        const jobs = [];
        try {
     ...
     */
    async cleanupArchives(olderThanDays = 90, archiveDir = './data/archived-jobs') {
