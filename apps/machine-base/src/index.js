#!/usr/bin/env node
/**
 * Base Machine Entry Point
 * Foundation for all EmProps machine types (GPU, API, etc.)
...
const logger = createLogger('base-machine');
 ...
 */
async function main() {
  logger.info('Starting Base Machine...', {
    version: '1.0.0',
  ...
  try {
    ...
    logger.info(`Base Machine ready (${startupTime}ms)`, {
      machineType: config.machine.type,
      servicesStarted: await pm2Manager.list()
    });

  } catch (error) {
    logger.error('Failed to start Base Machine:', error);
    process.exit(1);
  }
}
