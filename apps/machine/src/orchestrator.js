...
export class ServiceOrchestrator extends EventEmitter {
  ...
  async start() {
    ...
    try {
      ...
      // Phase 1.7: Port cleanup before AI services
      logger.info('Phase 1.7: Cleaning up ports before AI services');
      await this.startupNotifier.notifyStep('port_cleanup_begin', { phase: 'port_cleanup' });
      await this.startService('port-cleanup');
      await this.startupNotifier.notifyServiceStarted('port-cleanup');
      await this.startupNotifier.notifyStep('port_cleanup_complete', { phase: 'port_cleanup' });
  ...
  async loadServiceClass(serviceName) {
    const serviceModules = {
      ...
      'port-cleanup': './services/port-cleanup-service.js',
