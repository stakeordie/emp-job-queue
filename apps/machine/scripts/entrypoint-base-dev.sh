...
deserialize_environment() {
    log_section "Deserializing Environment Variables"
    
    if [ -n "${EMP_ENV_DECRYPT_KEY:-}" ]; then
        # Encrypted environment deserialization
        log_info "Decrypting compressed environment variables..."
        
        local ENCRYPTED_FILE="/workspace/env.encrypted"
        if [ ! -f "$ENCRYPTED_FILE" ]; then
            log_error "❌ Encrypted environment file not found: $ENCRYPTED_FILE"
            return 1
        fi
        
        # Use Node.js for decryption, decompression, and JSON parsing
        local TEMP_ENV_FILE="/tmp/emp_env_vars.sh"
        node -e "
            ...
            try {
                const decryptKey = Buffer.from(process.env.EMP_ENV_DECRYPT_KEY, 'base64');
                const encryptedData = fs.readFileSync('/workspace/env.encrypted', 'utf8');
                ...
                // Verify HMAC
                const hmac = crypto.createHmac('sha256', decryptKey);
                ...
                const decipher = crypto.createDecipheriv('aes-256-cbc', decryptKey, iv);
                ...
                Object.entries(envVars).forEach(([key, value]) => {
                    // Escape quotes and backslashes for bash
                    const escapedValue = value.replace(/\\\\/g, '\\\\\\\\').replace(/\"/g, '\\\\\"');
                    console.log(\`export \${key}=\"\${escapedValue}\"\`);
                });
                
                console.error(\`✅ Decrypted and decompressed \${Object.keys(envVars).length} environment variables\`);
            } catch (error) {
                console.error('❌ Environment decryption failed:', error.message);
                process.exit(1);
            }
        " > "$TEMP_ENV_FILE" 2>&1
        ...
        if [ $? -eq 0 ]; then
            ...
            rm -f "$TEMP_ENV_FILE"
            log_info "✅ Environment decryption successful"
        else
        ...
        log_info "No encrypted environment found, using direct environment variables"
    fi
}
...
setup_environment() {
    ...
    # Then, deserialize any serialized environment variables
    deserialize_environment || exit 1
...
# =====================================================
main() {
    log_section "EMP Machine Starting - Development Mode"
    log_info "Entrypoint script version: 2.0.0-dev"
    ...
    # Development mode - stop here and give shell
    log_section "Development Mode - Setup Complete"
    log_info "🚀 All setup complete! Ready for manual testing."
    log_info ""
    log_info "To start the main application manually:"
    log_info "  cd /service-manager && node src/index-pm2.js"
    log_info ""
    log_info "Available debug commands:"
    log_info "  debug-redis.sh    - Test Redis connection"
    log_info "  nvidia-smi        - Check GPU status"
    log_info "  env | grep -E '(REDIS|WORKER|MACHINE)' - Check environment"
    log_info ""
    log_info "Dropping to shell..."
    
    # Drop to interactive shell instead of starting application
    exec bash
}
