...
export class ComfyUIConnector extends HybridConnector {
  ...
  constructor(connectorId: string) {
    ...
    const config: Partial<HybridConnectorConfig> = {
      ...
      settings: {
        ...
        message_timeout_ms: parseInt(process.env.WORKER_COMFYUI_WORKFLOW_TIMEOUT_SECONDS || '600') * 1000,
  ...
  async getAvailableModels(): Promise<string[]> {
    try {
      ...
      const checkpoints = (objectInfo as any)?.CheckpointLoaderSimple?.input?.required?.ckpt_name?.[0] || [];
  ...
  protected async handleHttpSubmissionResponse(jobData: JobData, responseData: unknown): Promise<void> {
    const promptId = (responseData as any).prompt_id;
    if (!promptId) {
      throw new Error('No prompt ID returned from ComfyUI');
    }
